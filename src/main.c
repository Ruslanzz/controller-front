...
/**
  ...
  *
  *  Узел включает габаритные огни при старте (выход L2, DRV2 — яркость
  *  скважностью ШИМ), управляет реле передних указателей поворота по
  *  CAN-командам ведущего (device_id 0x01) и светодиодной балкой на выходе L1
  *  (DRV1): яркость 0..100 % задаётся потенциометром LA42DWQ-22 на входе
...
int main(void)
{
  ...
  /* Запуск ШИМ балки (DRV1) и габаритов (DRV2), исходное состояние выходов. */
