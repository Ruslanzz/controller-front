...
int main(void)
{
   ...
   *
   *   1) Lighting_Init выставляет значения сравнения в «закрыто» — ДО того,
   *      как ШИМ попадёт на ноги, иначе на плечи ушли бы значения по
   *      умолчанию, а ноль на верхнем плече означает свет на полную;
   *   2) ШИМ запускается;
   *   3) и только теперь поднимается EN — один раз за всё время работы.
   *
   * Прежний порядок (ШИМ раньше Lighting_Init) давал вспышку балки и габаритов
   * на полную яркость в момент включения питания. Снимать EN после этого
   * нельзя: он открывает верхний P-канальный ключ ПОЛНОСТЬЮ, и записи в
   * регистры сравнения перестают на что-либо влиять.                        */
  Lighting_Init();
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_1);
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_2);
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_3);
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_4);
  Lighting_EnableDrivers();
   ...
   * яркости (EXT_ADC_1), датчики тока (NSM2012) и температуры. */
  HAL_ADC_Start_DMA(&hadc1, (uint32_t *)ADS_RES_BUFFER, 8);

  /* Ноль датчиков тока снимается при ПОДНЯТОМ EN и закрытых верхних плечах:
   * свет погашен, тока через датчики нет. Ждать «снятия EN», как это делается
   * на узлах с обычным мостом, здесь нельзя — снятый EN означает открытый
   * ключ. Заодно считается потолок измерения и решается, помещается ли под
   * него порог защиты. */
